...
if ($options['ie9fix']) {
    theme_bootstrap_alice_recurse_svgs($path, '', 'theme_bootstrap_alice_svgtool_ie9fix', $blacklist);

} else if ($options['noaspectratio']) {
    theme_bootstrap_alice_recurse_svgs($path, '', 'theme_bootstrap_alice_svgtool_noaspectratio', $blacklist);
...
function theme_bootstrap_alice_svgtool_ie9fix($file) {
    global $CFG;
...
function theme_bootstrap_alice_svgtool_noaspectratio($file) {
    global $CFG;
...
function theme_bootstrap_alice_recurse_svgs($bootstrap_alice, $sub, $filecallback, $blacklist) {
    if (is_dir("$bootstrap_alice/$sub")) {
        $items = new DirectoryIterator("$bootstrap_alice/$sub");
        foreach ($items as $item) {
            if ($item->isDot()) {
                continue;
            }
            $file = $item->getFilename();
            theme_bootstrap_alice_recurse_svgs("$bootstrap_alice/$sub", $file, $filecallback, $blacklist);
        }
        unset($item);
        unset($items);
        return;

    } else if (is_file("$bootstrap_alice/$sub")) {
        if (substr($sub, -4) !== '.svg') {
            return;
        }
        $file = realpath("$bootstrap_alice/$sub");
